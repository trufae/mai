...
func NewConfigOptions() *ConfigOptions {
	...
	defaultEmbedModel := os.Getenv("MAI_MODEL_EMBED")
	if defaultEmbedModel == "" {
		defaultEmbedModel = "all-minilm@ollama"
	}
...
func (r *REPL) handleInvalidConfigKey(key string) string {
	...
	fmt.Fprintf(&output, "Error: configuration key '%s' does not exist\r\n", key)
...
func (r *REPL) handleSetCommand(args []string) (string, error) {
	...
	if _, exists := r.configOptions.GetOptionInfo(key); !exists {
		...
		if strings.HasSuffix(key, ".") {
			var output strings.Builder
			prefix := key
			fmt.Fprintf(&output, "Configuration options starting with '%s':\r\n", prefix)
			found := false
			for _, opt := range r.configOptions.GetAvailableOptions() {
				if strings.HasPrefix(opt, prefix) {
					...
					fmt.Fprintf(&output, "  %-20s = %-15s (type: %s)\r\n", opt, status, optType)
					found = true
				}
			}
			if !found {
				fmt.Fprintf(&output, "No configuration options found starting with '%s'\r\n", prefix)
			}
			return output.String(), nil
		}
	...
	case "ai.deterministic":
		_ = r.configOptions.Set("ai.deterministic", value)
		return "", nil
	case "llm.rawmode":
		_ = r.configOptions.Set("llm.rawmode", value)
		return "", nil
	case "dir.promptfile":
		return "", r.loadSystemPrompt(value)
	case "llm.systemprompt":
		_ = r.configOptions.Set("llm.systemprompt", value)
		return "", nil
	case "ai.model":
	...
	case "chat.format":
		...
		_ = r.configOptions.Set("chat.format", valLower)
		return "", nil
	case "mcp.reason":
		valLower := strings.ToLower(value)
