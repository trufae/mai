...
func extractToolNameMarkdown(response string) string {
...
func GetToolsFromMarkdown(message string) ([]*Tool, error) {
		...
		if err == nil && response != "" {
			...
			if err != nil {
				...
				if actionIdx != -1 && nextStepIdx != -1 {
					...
					currentToolName := extractToolNameMarkdown(response)
...
func executeToolsInMessage(message string) (string, error) {
	...
	tools, err := GetToolsFromMarkdown(message)
	if err != nil {
		return "", err
	}
