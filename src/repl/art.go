...
// It prints a title bar filled with the title background color and
// a body where each line is split into a small "ribbon" art block and
// a text block with a darker background and white foreground.
func DebugBanner(title, text string) {
	...
	// Wrap the text into lines fitting bodyCols
	...
	for _, w := range words {
		if displayWidth(cur)+1+displayWidth(w) <= bodyCols {
			...
			// if single word longer than bodyCols, break it
